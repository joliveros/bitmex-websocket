...
class TestWebsocket(object):
    def test_connect_should_connect_ws(self, mocker):
        connect_websocket = mocker.patch(
            'bitmex_websocket.BitMEXWebsocket.connect')

        socket = BitMEXWebsocket()

        # connect_websocket.assert_called_once()

    # def test_build_websocket_url_w_heartbeat(self, mocker):
    #     connect_websocket = mocker.patch(
    #         'bitmex_websocket.BitMEXWebsocket.connect_websocket')
    #
    #     socket = BitMEXWebsocket(heartbeat=True)
